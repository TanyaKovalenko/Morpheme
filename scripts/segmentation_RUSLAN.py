...
with open('result_plus_one.csv', 'w') as fout:
    for w in WordMorphemeDicts.words():
        print w
        ...
        result_split = ''
        for t, sub in reversed(zip(res_key, res_sub)):
            result_split += sub + '(' + t + ') '
        # for
        print result_split
        
        if orig_split != result_split:
            errors_fout.write(orig_split + ' | ' + result_split + '\n')
        # if

        all_count += 1
        pass_count += result_split == orig_split

        dif_time = time.time() - start_time
        prec = (pass_count + 0.0) / all_count
        print "------------", dif_time, matches, prec, all_count

        fout.write(str(dif_time) + ',' + str(matches) + ',' + str(prec) + ',' + str(all_count) + '\n') 
        matches = 0
    # for
# with
"""
