...
int main()
{
    ...
    for(iterator it = pre_v.begin(); it != pre_v.end(); ++it) {
        for (iterator it2 = root_v.begin(); it2 != root_v.end(); ++it2) {
            to_check = *it + *it2; to_write = "=" + *it + "," + *it2;
            ...
            for (iterator it3 = suf_v.begin(); it3 != suf_v.end(); ++it3) {
                to_check = *it + *it2 + *it3; to_write = "=" + *it + "," + *it2 + "," + *it3;
                if (etalon_h.find(to_check) != etalon_h.end())
                {
                    out << to_check << " : " << to_write << std::endl;
                    etalon_h.erase(to_check);
                ...
                for (iterator it4 = end_v.begin(); it4 != end_v.end(); ++it4) {
                    to_check = *it + *it2 + *it3 + *it4; to_write = "=" + *it + "," + *it2 + "," + *it3 + ",-" + *it4;
                    if (etalon_h.find(to_check) != etalon_h.end())
                    {
                        out << to_check << " : " << to_write << std::endl;
                        etalon_h.erase(to_check);
                    ...
                    {
                        t = clock() - t;
                        std::cout << result << " , hit=" << hit << " , it_time= " << ((float)t)/CLOCKS_PER_SEC << " , sum_time= " << ((float)(clock() - cum_t))/CLOCKS_PER_SEC << std::endl;
                        result += 0.1;
                        t = clock();
